...
if (isset($_REQUEST['btn_save_kemasan'])) {
    ...
    $foto_kemasan=time().$_FILES["input_foto_kemasan"]['name'];
        if (move_uploaded_file($_FILES['input_foto_kemasan']['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'/toko/projectToko/projectToko/gambar/'
          .$foto_kemasan)) {
            $target_file=$_SERVER['DOCUMENT_ROOT'].'/toko/projectToko/projectToko/gambar/'.$foto_kemasan;
        ...
        <div class="table-header">
            ...
            <script>
                function cari() {
                    ...
                    window.location.href="kemasan.php?cari="+searchInputValue;
                }
            </script>
        </div>
            ...
            <table>
                ...
                <?php
                    if (isset($_GET['cari'])) {
                        ...
                        <tbody>
                <?php
                    $query="select * from kemasan WHERE LOWER(nama_kemasan) LIKE LOWER('%$cari%')";
                    $result=mysqli_query($koneksi, $query);
                    $no=1;
