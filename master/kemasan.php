...
if (isset($_REQUEST['btn_save_kemasan'])) {
    ...
    $foto_kemasan=time().$_FILES["input_foto_kemasan"]['name'];
        if (move_uploaded_file($_FILES['input_foto_kemasan']['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'/sibrownies/projectToko/gambar/'
          .$foto_kemasan)) {
            $target_file=$_SERVER['DOCUMENT_ROOT'].'/sibrownies/projectToko/gambar/'.$foto_kemasan;
